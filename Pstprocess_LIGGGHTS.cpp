...
int main()
{
	...
	float id = 0, x = 0, y = 0, z = 0, ux = 0, uy = 0, uz = 0, us = 0, vx = 0, vy = 0, vz = 0, m = 0, s = 0, s_old = 0, fx = 0, fy = 0, fz = 0, omx = 0, omy = 0, omz = 0, cxx = 0, cyy = 0, czz = 0, cxy = 0, cxz = 0, cyz = 0, r = 0, count = 0;
	//float xb = 0.02, yb = 0.001, zb = 0.001, xl = -0.01, xu = 0.01, yl = 0.0, yu = 0.03, zl = 0.0, zu = 0.03, tstart = 80000, tstop = 100000,Vc=0,rho=2500;
	float xb = 0.001, yb = 0.02, zb = 0.001, xl = -0.12, xu = 0.12, yl = -0.01, yu = 0.01, zl = -0.15, zu = 0.10, tstart = 40000, tstop = 100000, Vc = 0, rho = 2500;
	int flag = 0, bin = 0, i = 0, j = 0, k = 0, lc = 0, binold = 0, type = 0;
	...
	*/
	...
	while (flag)
	{
		...
		else if (flag)
		{
			...
			for (int i_2 = 0; i_2 < No_of_atoms; i_2++)
			{
				inFile >> id >> type >> x >> y >> z >> binold >> vx >> vy >> vz >> s_old >> fx >> fy >> fz >> cxx >> cyy >> czz >> cxy >> cxz >> cyz >> r; // reading values separated by spaces from input file,edit these quantities according to your input dump file.
				if (x < xl || y < yl || z < zl)
				{
					continue;
				}
			   ...
			   /*if (lc >= 99)
				...
				cxx += m * pow(vx, 2); // only contact stresses remains after these operations.
				cyy += m * pow(vy, 2);
				czz += m * pow(vz, 2);
				cxy += m * vx * vy;
				cxz += m * vx * vz;
